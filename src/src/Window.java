...
public class Window implements Runnable, ActionListener {  
	...
	private JMenuItem laplaceMenuItem = new JMenuItem("Laplacian Filter");
	...
	boolean cropped = false;
	 //botões para mudar matriz de convolução
	...
	public void run(){
		...
		frame.setLayout(new BorderLayout()); // sei la nï¿½o sei oq ï¿½ melhor
		...
		frame.setSize(600,600); //  tamanho padrï¿½o 
	...
	private void prepareMenus() {
		...
		laplaceMenuItem.addActionListener(this);
	}
	...
	public void actionPerformed(ActionEvent ev){
		...
		if(ev.getSource() == laplaceMenuItem){
	...
	private void applyLaplacianFilter() {
		...
		LaplacianFilter laplaceFilter = new LaplacianFilter(image);
	
			previousImages.add(image);
			image = laplaceFilter.LaplaceFilter();     // desenha a imagem nova tendo o array resultante da convoluï¿½ï¿½o
	...
	private void applyGaussianFilter() {
		...
		try {
			previousImages.add(image);
			image = gaussianFilter.writeOutputImage(array);     // desenha a imagem nova tendo o array resultante da convoluï¿½ï¿½o
		} catch (IOException e) {
			System.out.println("Exception dps de tentar escrever a image no label");
			e.printStackTrace();
		}
	...
	private void updateImage() {
		lblCurrentImage.setIcon(new ImageIcon(image)); //adiciona a imagem em outro label , ainda nï¿½o gosto disso mas parece unica opï¿½ï¿½o
	...
	private void openImage() {
		...
		if(val == JFileChooser.APPROVE_OPTION){
			...
			lblOriginalImage = new JLabel(new ImageIcon(image)); // botando a imagem original num label. ???? alguma coisa melhor pra isso ???? - tem que ter , ï¿½ocorro.
